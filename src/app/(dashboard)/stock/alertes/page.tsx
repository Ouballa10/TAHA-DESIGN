  ...
  return (
    <div className="space-y-5">
      <PageHeader
        eyebrow="Alertes"
        title="Stock bas"
        description="Les references ci-dessous ont atteint ou depasse leur seuil minimal."
        actions={
          <Link
      ...
      {variants.length === 0 ? (
        <EmptyState
          title="Aucune alerte"
          description="Tout le stock est au-dessus des seuils definis."
        />
      ) : (
          ...
          {variants.map((variant) => (
            <div key={variant.variant_id} className="rounded-[2rem] border border-border bg-white/70 p-5">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-3">
                ...
                <div>
                  <p className="text-xs text-muted">Seuil</p>
                  <p className="font-semibold text-foreground">{variant.minimum_stock}</p>
                </div>
                <div>
                  <p className="text-xs text-muted">Prix achat</p>
